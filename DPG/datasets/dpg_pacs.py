...
class DPG_PACS(DatasetBase):
    ...
    def __init__(self, cfg):
        ...
        # self.split_dir = osp.join(self.dataset_dir, "dpg_coop_splits", str(cfg.SEED))
    ...
    def _read_data(self, source_domains, target_domain):
        ...
        for domain, dname in enumerate(source_domains):
            ...
            if dname not in train:
                train[dname] = []
                val[dname] =[]

            train[dname].extend(split_train)
            val[dname].extend(split_val)
        ...
        test[target_domain] = []
        test[target_domain].extend(split_train)
        test[target_domain].extend(split_val)
