...
class BaseTextEncoder(nn.Module):
    ...
    def forward(self, prompts, tokenized_prompts):
        ...
        for i in range(len(prompts)):
            x = prompts[i] + self.positional_embedding.type(prompts[i].dtype)
            x = x.permute(1, 0, 2)  # NLD -> LND
            x = self.transformer(x)
            x = x.permute(1, 0, 2)  # LND -> NLD
            x = self.ln_final(x).to(self.dtype)

            batch_x.append(x[torch.arange(x.shape[0]), tokenized_prompts.argmax(dim=-1)] @ self.text_projection)

        batch_x = torch.stack(batch_x).type(prompts.dtype)
